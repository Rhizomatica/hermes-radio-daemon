...
/* Own limits, not rade_constants.h's V1-sized ones: the V2 decoder has 128-neuron
   GRUs and conv layers with up to 1792 inputs (see upstream opus-nnet.c.diff). */
#define RADE_NNET_MAX_RNN_NEURONS 256
#define RADE_NNET_MAX_CONV_INPUTS 2048
...
void compute_generic_gru(const LinearLayer *input_weights, const LinearLayer *recurrent_weights, float *state, const float *in, int arch) {
    int N;
    float zrh[3 * RADE_NNET_MAX_RNN_NEURONS];
    float recur[3 * RADE_NNET_MAX_RNN_NEURONS];
    float *z;
    float *r;
    float *h;
    ...
    assert(N <= RADE_NNET_MAX_RNN_NEURONS);
...
void compute_generic_conv1d(const LinearLayer *layer, float *output, float *mem, const float *input, int input_size, int activation, int arch) {
    float tmp[RADE_NNET_MAX_CONV_INPUTS];
    ...
    assert(input != NULL);
    assert(input != output);
    assert(layer->nb_inputs <= RADE_NNET_MAX_CONV_INPUTS);
